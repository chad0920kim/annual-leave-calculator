...
# Extra pre-verified (HTTP 200) Pexels photo IDs, kept in reserve so a fresh,
# never-before-used image is available before we have to fall back to
# reusing one from an existing post. Add more here as the pool runs low.
EXTRA_IMAGE_IDS = [
    3184291, 3183197, 3823488, 3861969, 5989927,
    4098365, 3760067, 5439381, 3184339, 3182773,
]
EXTRA_IMAGE_POOL = [
    f"https://images.pexels.com/photos/{i}/pexels-photo-{i}.jpeg?auto=compress&cs=tinysrgb&w=1200"
    for i in EXTRA_IMAGE_IDS
]
...
def pick_image(posts):
    used = {p["image"] for p in posts}

    # Prefer a genuinely unused image over reusing one from an existing post.
    fresh = [url for url in EXTRA_IMAGE_POOL if url not in used]
    if fresh:
        return fresh[0]

    # Pool exhausted: fall back to whichever image was used longest ago.
    last_used_index = {}
